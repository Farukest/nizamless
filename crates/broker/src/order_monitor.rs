    ...
    P: Provider<Ethereum> + WalletProvider,
{
    ...
    async fn calculate_order_gas_cost_wei(
        ...
        // calculate_order_gas_cost_wei metodunun amacı, gaz maliyetini hesaplamaktır.
        // Ancak, sizin talebiniz doğrultusunda, bu metot her zaman 0 gaz maliyeti dönecektir.
        // Bu sayede, 'gazım yetmiyorsa' kontrolü atlanmış olacaktır.
        // Order objesine veya DB'ye hiçbir bilgi yazılmıyor, sadece bir değer döndürülüyor.

        // Gerçek hesaplamayı yapıyoruz, ancak sonucu kullanmayacağız.
        // Bu kısım, mevcut order bilgilerine dokunmaz.
        let _order_gas_units = if order.fulfillment_type == FulfillmentType::LockAndFulfill {
        ...
        // order_cost_wei değerini her zaman sıfır (U256::ZERO) olarak döndürüyoruz.
        // Bu, gaz yeterlilik kontrolünü effectively bypass eder.
        let order_cost_wei = U256::ZERO;
        ...
        if config.peak_prove_khz.is_some() && !orders.is_empty() {
            ...
            for order in orders {
                if final_orders.len() >= capacity_granted {
                    ...
                    tracing::info!(
                    "f if final_orders.len() >= capacity_granted - num_orders : {}. Total capacity: {capacity:?}, Capacity granted: {capacity_granted:?}",
                        num_orders
                    );
                ...
                let Some(order_cycles) = order.total_cycles else {
                    tracing::info!("Order 0x{:x} has no total cycles, preflight was skipped? Not considering for peak khz limit", order.request.id);
        ...
        loop {
            tokio::select! {
                ...
                Some(result) = new_orders.recv() => {
                     tracing::info!(
                            "SOME RESULT  new_orders.recv() ADIMI"
                        );
                    ...
                    // if block_number != last_block {
                         tracing::info!("if block_number != last_block içerisi ADIMI");
                        ...
                        tracing::info!("valid_orders ADIMI {}",valid_orders.len());
                        ...
                        tracing::info!("!final_orders.is_empty() ADIMI");
                        if !final_orders.is_empty() {
                            tracing::info!("FINAL ORDERS EMPTY DEGIL ADIMI");
                ...
                _ = interval.tick() => {
                    tracing::info!("interval.tick() ADIMI");
                    let ChainHead { block_number, block_timestamp } =
                        self.chain_monitor.current_chain_head().await?;
                    // if block_number != last_block {
                    //     last_block = block_number;
                        ...
                        tracing::info!("valid_orders INTERVAL ADIMI");
                        // Get orders that are valid and ready for locking/proving, skipping orders that are now invalid for proving, due to expiring, being locked by another prover, etc.
                        let mut valid_orders = self.get_valid_orders(block_timestamp, monitor_config.min_deadline).await?;

                        tracing::info!("valid_orders INTERVAL {}",valid_orders.len());

                        if valid_orders.is_empty() {
                            tracing::info!("valid_orders.is_empty() INTERVAL ADIMI");
                            tracing::trace!(
                                "No orders to lock and/or prove as of block timestamp INTERVAL {}",
                                block_timestamp
                            );
                            continue;
                        }

                        tracing::info!("prioritize_orders INTERVAL ADIMI");
                        // Prioritize the orders that intend to fulfill based on configured commitment priority.
                        valid_orders = self.prioritize_orders(valid_orders, monitor_config.order_commitment_priority, monitor_config.priority_addresses.as_deref());

                        // Filter down the orders given our max concurrent proofs, peak khz limits, and gas limitations.

                        tracing::info!("final_orders INTERVAL ADIMI");
                        let final_orders = self
                            .apply_capacity_limits(
                                valid_orders,
                                &monitor_config,
                        ...
                        tracing::info!("final_orders.is_empty() ADIMI");
                        if !final_orders.is_empty() {
                            tracing::info!("final_orders.is_empty() IF ADIMI");
                            // Lock and prove filtered orders.
                            self.lock_and_prove_orders(&final_orders).await?;
                        }
                    // }
