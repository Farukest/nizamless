    ...
    P: Provider<Ethereum> + 'static + Clone + WalletProvider,
{
    ...
    pub fn new(
        ...
        Self {
            ...
            prover,
            provider,
            lock_and_prove_cache: Arc::new(Cache::builder().expire_after(self::OrderExpiry).build()),
            chain_monitor,
            market,
    ...
    /// Spawns a task to price an order and update its state in the database.
    /// This method simplifies profitability checks, leaving more complex capacity management
    /// to the OrderMonitor.
    async fn price_order_and_update_state(
        ...
        let f = || async {
            ...
            match pricing_result {
                Ok(OrderPricingOutcome::Lock { total_cycles, target_timestamp_secs, expiry_secs }) => {
                    ...
                    tracing::info!(
                        "Order {order_id} preflighted. Ready for lock and fulfill. Target: {}, Expiry: {}",
                ...
                Ok(OrderPricingOutcome::ProveAfterLockExpire {
                    ...
                    tracing::info!("Setting order {order_id} preflighted. Ready to prove after lock expiry at {lock_expire_timestamp_secs}");
                    order.total_cycles = Some(total_cycles);
                ...
                Ok(OrderPricingOutcome::Skip) => {
                    tracing::info!("Skipping order {order_id} after preflight checks.");
    ...
    /// Prices an order by performing preflight and basic validity checks.
    /// This function focuses on determining if an order is viable for proving,
    /// rather than explicit profitability or locking.
    async fn price_order(
        ...
        let (allowed_addresses_opt, denied_addresses_opt) = {
            let config = self.config.lock_all().context("Failed to read config")?;
            (
        ...
        let expiration = if lock_expired {
            order_expiration
        } else {
            lock_expiration
        };
        ...
        let max_stake = {
            let config = self.config.lock_all().context("Failed to read config")?;
            let max_stake_str = &config.market.max_stake;

            // 'max_stake' değeri broker.toml'da "1" gibi okunacak ve bu 1 USDC'ye denk gelecek.
            // USDC'nin genellikle 6 ondalık hanesi vardır, bu da 1 USDC'nin 1,000,000 (10^6) en küçük birime eşit olduğu anlamına gelir.
            let max_stake_usdc_value = max_stake_str.parse::<U256>()
                .context("Failed to parse max_stake config value as U256. Ensure it's a valid number string.")?;

            // USDC için 6 ondalık hane çarpanı (10^6)
            let usdc_decimals_multiplier = U256::from(1_000_000);

            // USDC değerini en küçük token birimine çeviriyoruz
            max_stake_usdc_value.checked_mul(usdc_decimals_multiplier)
                .context("Calculated max_stake value overflowed U256. The configured max_stake might be too large.")?
        };

        let lockin_stake = U256::from(order.request.offer.lockStake);

        tracing::info!("---------- lockin_stake {} : ", lockin_stake);
        tracing::info!("---------- max_stake {} : ", max_stake);
        if lockin_stake > max_stake {
            tracing::info!("---------- lockin_stake if {} : ", lockin_stake);
            tracing::info!("---------- lockin_stake if {} : ", max_stake);
            tracing::info!("Removing high stake order {order_id}, lock stake: {lockin_stake}, max stake: {max_stake}");
            return Ok(Skip);
        }

        // YENİ DÜZENLENMİŞ KOD BURADAN BAŞLIYOR (Tip hatası düzeltildi)
        // Lock Timeout süresi min_allowed_lock_timeout_secs dakikadan AZ olan siparişleri filtrele (alma)
        let min_allowed_lock_timeout_secs = {
            let config = self.config.lock_all().context("Failed to read config")?;
            config.market.min_lock_out_time * 60
        };

        tracing::info!("---------- min_allowed_lock_timeout_secs {} : ", min_allowed_lock_timeout_secs);

        // order.request.offer.lockTimeout'u u64'e dönüştürerek karşılaştırma yapıyoruz.
        if (order.request.offer.lockTimeout as u64) < min_allowed_lock_timeout_secs { // Buradaki operatör ve dönüşüm doğru!
            tracing::info!(
                "Skipping order {}: Lock Timeout ({} seconds) is less than minimum required ({} seconds).",
                order.request.id,
                order.request.offer.lockTimeout, // Burayı değiştirmeden u32 olarak loglayabiliriz, sorun değil.
                min_allowed_lock_timeout_secs
            );
            return Ok(Skip);
        }
        // YENİ DÜZENLENMİŞ KOD BURADA BİTİYOR
        ...
        // Calculate estimated gas cost for preflight exec limit if needed, even though
        // the actual gas check is in OrderMonitor. This is to ensure preflight is not
        // given an unrealistic cycle limit.
        let gas_price = 30000000u64;
        ...
        tracing::info!(
            "Estimated {order_gas} gas to {} order {order_id} (for preflight limit calc); {} ether @ {} gwei",
        ...
        tracing::info!("---------- ORDERPICKER gas_price {} : ", gas_price);
        tracing::info!("---------- ORDERPICKER order_gas {} : ", order_gas);
        tracing::info!("---------- ORDERPICKER order_gas_cost {} : ", order_gas_cost);
        ...
        if skip_mcycle_limit {
            ...
            tracing::info!("Order {order_id} exec limit skipped due to client {} being part of priority_requestor_addresses.", client_addr);
            ...
            if exec_limit_cycles >= config_cycle_limit {
                tracing::info!("Order {order_id} exec limit computed from max price {} exceeds config max_mcycle_limit {}, setting exec limit to max_mcycle_limit", exec_limit_cycles / 1_000_000, config_mcycle_limit);
        ...
        if exec_limit_cycles == 0 {
            tracing::info!("Order {order_id} has no time left to prove within deadline, skipping");
            return Ok(OrderPricingOutcome::Skip);
        }
        ...
        // peak_prove_khz bir Option<u64> olduğu için, değerini kullanmadan önce açmalıyız.
        // .unwrap_or(1) ile varsayılan olarak 1 değerini veriyoruz, eğer config'de belirtilmemişse.
        let current_peak_prove_khz = peak_prove_khz.unwrap_or(1); // <-- Burası hatayı çözen satır
        tracing::info!("current_peak_prove_khz {} : ", current_peak_prove_khz);
        // ... (diğer URI yükleme ve gas hesaplama kısımları) ...

        // Siparişin kalan geçerlilik süresini hesaplayalım
        let now = now_timestamp();

        let effective_deadline = order.request.offer.biddingStart + order.request.offer.lockTimeout as u64;

        if let Some(existing_target_ts) = order.target_timestamp {
            if existing_target_ts != effective_deadline {
                ...
                order.request.id, effective_deadline, existing_target_ts);
            }
        }

        let available_time_for_order_secs = effective_deadline.saturating_sub(now);

        tracing::info!("--  order.request.id 0x{:x} : ",  order.request.id);
        tracing::info!("-- nowww {} : ", now);
        tracing::info!("-- current_peak_prove_khz {} : ", current_peak_prove_khz);
        tracing::info!("-- expirationnn {} : ", expiration);
        tracing::info!("-- effective_deadlineee {} : ", effective_deadline);
        tracing::info!("-- order.request.offer.biddingStart {} : ", order.request.offer.biddingStart);
        tracing::info!("-- order.request.offer.lockTimeout {} : ", order.request.offer.lockTimeout);
        tracing::info!("-- available_time_for_order_secsss {} : ", available_time_for_order_secs);
        ...
        tracing::info!("-- simulated_proof_time_secs {} : ", simulated_proof_time_secs);

        // peak_prove_khz değerini kullanarak tahmini cycle sayısını hesaplayalım.
        // Burada 'peak_prove_khz' yerine 'current_peak_prove_khz' kullanıyoruz.
        let mut simulated_total_cycles = simulated_proof_time_secs.saturating_mul(current_peak_prove_khz * 1_000); // <-- Değişen satır

        // Minimum bir cycle sınırı koymak iyi bir pratik olabilir.
        simulated_total_cycles = simulated_total_cycles.max(2); // En az 2 cycle olmalı
        tracing::info!("simulated_total_cycles {} : ", simulated_total_cycles);


        // Hesaplanan simulated_total_cycles değerini max_mcycle_limit ile sınırlayalım.
        ...
        tracing::info!("simulated_total_cycles 2 {} : ", simulated_total_cycles);

        order.total_cycles = Some(simulated_total_cycles);


        match order.fulfillment_type {
            FulfillmentType::LockAndFulfill => Ok(OrderPricingOutcome::Lock {
                total_cycles: order.total_cycles.unwrap_or_default(),
                target_timestamp_secs: order.request.lock_expires_at(), // Use lock_expires_at as target
                expiry_secs: order.request.expires_at(),
            }),
            FulfillmentType::FulfillAfterLockExpire => Ok(OrderPricingOutcome::ProveAfterLockExpire {
                total_cycles: order.total_cycles.unwrap_or_default(),
                lock_expire_timestamp_secs: order.request.expires_at(), // Use expires_at as target for this type
                expiry_secs: order.request.expires_at(),
            }),
            _ => {
                // This case should ideally be caught earlier by supported_selectors or other checks
                tracing::warn!("Unsupported fulfillment type for order {order_id}: {:?}", order.fulfillment_type);
                Ok(OrderPricingOutcome::Skip)
            }
        }
    }
